...
This approach relies heavily on the [mlr package](https://mlr.mlr-org.com/) [@bischl_mlr:_2016] which is an unified interface to perform machine learning analysis using R. We strongly recommand you to read to mlr documentation in order to understand its terminology and principles. Doing so you'll also get knowledge about machine learning theory. In this package we will refer a lot to the mlr terminology. Each of the terms we will use here must be understood in terms of their mlr definition (e.g. learner, benchmark experiment, etc.) 
...
The respective __performance__ of multiple __explorative constructions__ are assessed by performing a batch of benchmarks experiments. A new __milestone__ is reached each time a new explorative construction has been integrated to a batch of benchmark experiments. A __best construction reference__ is defined by the list of the ID's of the considered explorative constructions injected into the batch of benchmark experiments and the ID of the best explorative construction among this list. 

When spatializing data using the best investigated __explorative construction__ of a __milestone__ , its __best construction reference__ must be stored as a metadata of the spatialized data
