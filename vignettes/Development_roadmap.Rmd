...
A specific attention will be ported on the analysis of the quality of the data produced by each of our stations. We will need to carry analysis in order to detect eventual structural or local effects such as overheating in temperature shelters. 
...
The Agromet project will spatialize weather data gathered both by the [Pameseb](https://www.pameseb.be/) network owned by the [CRA-W](http://www.cra.wallonie.be/fr) and stations owned by the national weather office [RMI](https://www.meteo.be). 
...
* __elevation__ : the dataset if freely available on the [Registry of Open Data on AWS](https://registry.opendata.aws/terrain-tiles/) and can be easily donwloaded using the [elevatr](https://github.com/jhollist/elevatr) package 
* __slope__ : this variable is computed from elevation using the raster package
* __aspect__ : this variable is computed from elevation using the raster package
* __soil occupation__ : this variable is downloaded form the [NGI website](http://inspire.ngi.be/download-free/atomfeeds/AtomFeed-en.xml)
...
We have decided to group the algorithms in various categories of increasing complexity. The source code used to construct the learners from these algorithms is stored into the `data-raw/makeLearners.R` file of the present package. These learners are constructed on the basis of the `mlr` package.
...
This approach will be based on the [TensorFlow](https://tensorflow.rstudio.com/) library but will not be considered before 2020. Introduction in the [Machine Learning with R and TensorFlow video](https://www.youtube.com/watch?v=atiYXm7JZv0&t=73s).

# Milestones

Multiple combinations of the precedents parameters will be tested in various batch of benchmarks experiments. Each of them represent one of our milestones.

## milestone1

```{r, milestone1, echo = FALSE}
id = c("baseMulLR", "baseIDW", "base1NN", "baseOK", "baseKED")
algorithms = c('mulLR','IDW','1NN', 'OK', 'KED')
explanatory = c("alt, lon, lat", "lon, lat", "lon, lat", "lon, lat", "alt, lon, lat")
...
target = c("tsa", "tsa", "tsa", "tsa", "tsa")


milestone1 = data.frame(id, algorithms, explanatory, hyperparameters, stations, resolution, target)
knitr::kable(milestone1)
```

## milestone2

Same as milestone 1 instead the time resolution that is daily and not hourly
...
hyperparameters = c(NA, NA, NA, "range = 800, psill = 200000, model.manual = 'Sph', nugget = 0", "range = 800, psill = 200000, model.manual = 'Sph', nugget = 0")
stations = c("Pameseb", "Pameseb", "Pameseb", "Pameseb", "Pameseb")  
resolution = c("daily", "daily", "daily", "daily", "daily")
target = c("tsa", "tsa", "tsa", "tsa", "tsa")

milestone2 = data.frame(id, algorithms, explanatory, hyperparameters, stations, resolution, target)
...
stations = c("Pameseb", "Pameseb", "Pameseb", "Pameseb", "Pameseb")  
resolution = c("daily", "daily", "daily", "daily", "daily")
target = c("hra", "hra", "hra", "hra", "hra")


milestone2 = data.frame(id, algorithms, explanatory, stations, resolution, target)
knitr::kable(milestone2)
```
