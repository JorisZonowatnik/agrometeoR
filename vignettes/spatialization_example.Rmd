...
# create the task
myTasks = makeTasks(dataset = (makeDataset(dfrom = "2017-03-04T15:00:00Z", dto = "2017-03-04T15:00:00Z", sensor = "tsa"))$output$value, target = "tsa")
# what are the variables ? 
colnames(mlr::getTaskData(myTasks$output$value$`20170304150000`))
# what data do we have ? 
head(getTaskData(myTasks$output$value$`20170304150000`))
```
...
myModel = makeModel(task = myTasks$output$value$`20170304150000`, learner = myLearner)
# what are the model performances ? 
myModel$output$value$perfs$agg
# what about the residuals ? 
plot(x =  getTaskData(myTasks$output$value$`20170304150000`)[["tsa"]], myModel$output$value$residuals[[1]])
```
...
files = list.files()
if (!is.null(paste0(myTasks$output$value$`20170304150000`$task.desc$id, ".geojson" %in% files))) {
  file.remove(paste0(myTasks$output$value$`20170304150000`$task.desc$id, ".geojson"))
}
```
